...
public class Paths {
    ...
    public static <T> List<Node> dijkstra(Node start, Node end) {
        // TODO Implement Dijktras's shortest-path algorithm as presented
        ...
        map.put(start, new NodeInfo(null, 0));
        
        while (frontier.size() != 0) {
        	Node f = frontier.poll();
        	
        	for (Edge neighbors : f.getExits()) {
        		Node w = neighbors.getOther(f);
        		if (!map.containsKey(w)) {
        			int lengthW = map.get(f).distance + neighbors.length;
        			frontier.add(w, lengthW);
        			map.put(w, new NodeInfo(f, map.get(f).distance + neighbors.length));
        		}
        		else {
        			if (map.get(f).distance + neighbors.length < map.get(w).distance) {
        				map.put(w, new NodeInfo(f, map.get(f).distance + neighbors.length));
        				frontier.changePriority(w, map.get(w).distance);
        			}
        		}
        	}
